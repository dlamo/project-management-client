...
class ProjectDetails extends Component {
    ...
    getSingleProject = () => {
        ...
        axios.get('http://localhost:3002/api/projects/' + params.id)
        .then(response => {
    ...
    deleteProject = () => {
        const { params } = this.props.match;
        axios.delete(`http://localhost:3002/api/projects/${params.id}`)
        .then( () =>{
    ...
    render() {
        return (
            <div>
                ...
                <button style={{color: 'red'}} onClick={() => this.deleteProject()}>Delete project</button><br />
